...
- 认证
  - `POST /api/v1/auth/login` 登录获取访问令牌
  - `GET /api/v1/auth/me` 使用 Bearer Token 获取当前用户信息
- 首页统计
  - `GET /api/v1/stats` 返回成员、项目、分享会、设备、文件的数量统计
- 文件管理（部分）
  - `GET /api/v1/files` 列表，返回 `{ data, total }`
  - `POST /api/v1/files/upload` 上传单个文件
  - `GET /api/v1/files/{id}` 详情
  - `GET /api/v1/files/{id}/download` 下载
  - `PUT /api/v1/files/{id}` 更新
  - `DELETE /api/v1/files/{id}` 删除
  - `POST /api/v1/files/batch-upload` 批量上传（最多10个）
  - `GET /api/v1/files/stats` 文件统计（总数、总大小、类型分布）

> 更多端点与请求示例请参考 `docs/GETTING_STARTED.md`。

提示：首次登录会自动创建或重置默认管理员（用户名/密码来自 `.env` 或默认 `admin/admin123`）。

前端令牌使用：在 `frontend/src/utils/request.js` 的请求拦截器中为需要认证的请求添加 `Authorization: Bearer <token>`。

### 说明

- 批量上传接口 `POST /api/v1/files/batch-upload` 统一返回 `BatchUploadResponse`：`{ uploaded_files, failed_files, message }`
